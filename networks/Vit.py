...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print(context_layer.dtype, context_layer.shape)
        # exit()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, config, img_size=224, hash_bit=16,nclass=0):
        ...
        # self.hash_layer = nn.Linear(768, hash_bit)
        self.hash_layer = nn.Linear(self.num_classes, hash_bit)
        # self.classifier = nn.Linear(25216, num_classes)

        self.part_head10 = HashLayer(hash_bit=hash_bit)
        self.part_head11 = HashLayer(hash_bit=hash_bit)
        self.part_head12 = HashLayer(hash_bit=hash_bit)
        self.part_head13 = HashLayer(hash_bit=hash_bit)

        self.cls = nn.Sequential(
            nn.ReLU(),
            nn.Linear(config.hidden_size, nclass),
            nn.Softmax()
        )
        self.p = config.p
    ...
    def forward(self, x, swap=False):
        # print(x.shape)
        out10_, out11_, out12_, out13_, weight = self.transformer(x)

        out10 = self.part_head10(out10_[:, 0])
        out11 = self.part_head11(out11_[:, 0])
        out12 = self.part_head12(out12_[:, 0])
        out13 = self.part_head13(out13_[:, 0])
        # fea_hash = self.hash_layer(out13)
        cls_head = self.cls(out13_[:, 0])
        # fea_cls = self.classifier(out13)
        # fea_hash = self.hash(out13)
        if swap:
            batch_size = x.size()[0]
            image = x
            ...
            return out10, out11, out12, out13, image_new, lam_a, lam_b, rand_index, cls_head
        else:
            return out10, out11, out12, out13,cls_head
            # return fea_hash, fea_cls, out13
        # else:
        #     return fea_hash, fea_cls, out13
