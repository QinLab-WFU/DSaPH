...
from networks.utils import batch_index_select,get_index
import numpy as np
import math

from zoom import num_classes
...
class CFVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size_list=[112, 224], patch_size=16, in_chans=3, num_classes=num_classes, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm,hash_bit=None):
        super().__init__()
        ...
        # Hash
        self.hash_fc = nn.Linear(embed_dim, hash_bit)
    ...
    def forward(self, xx):
        ...
        # print("Before operation: ", x.shape)
        ...
        hash_result.append(self.hash_fc(x[:, 0]))
...
@register_model
def cf_deit_small(pretrained=False, hash_bit=None, num_class=None, **kwargs):
    model = CFVisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),hash_bit=hash_bit,num_classes=num_class, **kwargs).cuda()
    return model

if __name__ == '__main__':
    model = cf_deit_small(pretrained=True, hash_bit=16, num_class=17)
    ...
    for i in input_size_list:
        resized_img = F.interpolate(test,
                                    (i,i),
                                    mode='bilinear',
                                    align_corners=True
                                    )
